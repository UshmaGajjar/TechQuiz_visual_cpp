...
langc::langc(CWnd* pParent /*=NULL*/)
	: CDialog(langc::IDD, pParent)
{
	//{{AFX_DATA_INIT(langc)
	//}}AFX_DATA_INIT
}
...
void langc::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(langc, CDialog)
	//{{AFX_MSG_MAP(langc)
...
/////////////////////////////////////////////////////////////////////////////
// langc message handlers

void langc::OnOption1() 
{
	m_opt2.SetCheck(0);
	m_opt3.SetCheck(0);
	m_opt4.SetCheck(0);
...
BOOL langc::OnInitDialog() 
{
	CDialog::OnInitDialog();
	
	db.OpenEx(_T("DSN=quiz_dsn;UID=admin"),CDatabase::noOdbcDialog);
	rs=new CRecordset(&db);
	rs->Open(CRecordset::dynaset,_T("select * from c_tb"),CRecordset::none);
	
	db.ExecuteSQL("update c_tb set user_ans=0"); 
...
void langc::DisableNextButton()
{
	m_next.EnableWindow(false);
}

void langc::OnTimer(UINT nIDEvent) 
{
	UpdateData(true);
...
void langc::InsertAnswer(int ans)
{
	CString temp1,temp2;
	...
	db.ExecuteSQL("Update c_tb set user_ans="+ temp2 +" where qno="+ temp1 +";");
}

void langc::EnableNextButton()
{
