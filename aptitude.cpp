// aptitude.cpp : implementation file
//
...
aptitude::aptitude(CWnd* pParent /*=NULL*/)
	: CDialog(aptitude::IDD, pParent)
{
	//{{AFX_DATA_INIT(aptitude)
		// NOTE: the ClassWizard will add member initialization here
	//}}AFX_DATA_INIT
}


void aptitude::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(aptitude)
...
BEGIN_MESSAGE_MAP(aptitude, CDialog)
	//{{AFX_MSG_MAP(aptitude)
...
/////////////////////////////////////////////////////////////////////////////
// aptitude message handlers

void aptitude::OnOption1() 
{
	m_opt2.SetCheck(0);
	m_opt3.SetCheck(0);
	m_opt4.SetCheck(0);
...
BOOL aptitude::OnInitDialog() 
{
	CDialog::OnInitDialog();
	
	db.OpenEx(_T("DSN=quiz_dsn;UID=admin"),CDatabase::noOdbcDialog);
	rs=new CRecordset(&db);
	rs->Open(CRecordset::dynaset,_T("select * from aptitude_tb"),CRecordset::none);

	db.ExecuteSQL("update aptitude_tb set user_ans=0");
...
void aptitude::EnableNextButton()
{
...
void aptitude::OnTimer(UINT nIDEvent) 
{
	UpdateData(true);
...
void aptitude::OnResult() 
{
	UpdateData(true);
	
	pbar.tb_name="aptitude_tb";

	InsertAnswer(uans);
	aptitude::EndDialog(0);
	pbar.DoModal();
	
	UpdateData(false);	
}

void aptitude::UnCheckAll()
{
	 m_opt1.SetCheck(0);
	m_opt2.SetCheck(0);
	m_opt3.SetCheck(0);
...
void aptitude::DisableNextButton()
{
	  m_next.EnableWindow(false);
}

void aptitude::InsertAnswer(int ans)
{
	 CString temp1,temp2;
	...
	db.ExecuteSQL("Update aptitude_tb set user_ans="+ temp2 +" where qno="+ temp1 +";");

}

void aptitude::OnQuit() 
{
