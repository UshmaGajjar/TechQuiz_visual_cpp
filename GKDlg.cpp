...
GKDlg::GKDlg(CWnd* pParent /*=NULL*/)
	: CDialog(GKDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(GKDlg)
	//}}AFX_DATA_INIT
}
...
void GKDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(GKDlg, CDialog)
	//{{AFX_MSG_MAP(GKDlg)
...
void GKDlg::InsertAnswer(int ans)
{
	 CString temp1,temp2;
	...
	db.ExecuteSQL("Update gk_tb set user_ans="+ temp2 +" where qno="+ temp1 +";");

}

void GKDlg::OnTimer(UINT nIDEvent) 
{
	UpdateData(true);
...
BOOL GKDlg::OnInitDialog() 
{
	...
	rs->Open(CRecordset::dynaset,_T("select * from gk_tb"),CRecordset::none);
	
	db.ExecuteSQL("update gk_tb set user_ans=0"); 
...
void GKDlg::DisableNextButton()
{
	  m_next.EnableWindow(false);

}

void GKDlg::EnableNextButton()
{
